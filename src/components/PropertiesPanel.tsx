...
import { edgeRouting, resolveEndpointPoint } from '../core/routing';
...
function EmptyProperties({ document, page }: { document: DiagramDocument; page?: DiagramPage }) {
  ...
  return <aside className="properties-panel empty-properties">
     <InspectorHeading />
     <div className="no-selection"><div className="no-selection-art"><div /><div /><div /></div><strong>Nothing selected</strong><span>Select an object to inspect its properties.</span></div>
     {settings && <InspectorSection id="page.settings" title="Page"><label className="field-label">Background<div className="page-background-field"><input aria-label="Page background" type="color" value={/^#[0-9a-f]{6}$/i.test(settings.background) ? settings.background : '#10131c'} onChange={(event) => updatePageSettings({ background: event.target.value }, page?.id, 'Change page background')} /><span>{settings.background}</span></div></label><div className="coordinate-grid"><label className="field-label">Grid size<input aria-label="Grid size" type="number" min="1" max="240" value={settings.gridSize} onChange={(event) => updatePageSettings({ gridSize: Math.max(1, Number(event.target.value) || settings.gridSize) }, page?.id, 'Change grid size')} /></label><label className="field-label">Page size<span className="page-setting-value">{Math.round(settings.width)} × {Math.round(settings.height)}</span></label></div><div className="empty-setting-toggles"><div className="entity-style-toggle"><span>Grid</span><button className={settings.gridVisible ? 'toggle-button active' : 'toggle-button'} aria-label="Toggle page grid" aria-pressed={settings.gridVisible} onClick={() => updatePageSettings({ gridVisible: !settings.gridVisible }, page?.id, 'Toggle grid')}><span /></button></div><div className="entity-style-toggle"><span>Snap to grid</span><button className={settings.snapToGrid ? 'toggle-button active' : 'toggle-button'} aria-label="Toggle page snapping" aria-pressed={settings.snapToGrid} onClick={() => updatePageSettings({ snapToGrid: !settings.snapToGrid }, page?.id, 'Toggle snap')}><span /></button></div></div></InspectorSection>}
...
function EdgePropertiesPanel({ edge, nodes, diagramType, document }: { edge: DiagramEdge; nodes: DiagramNode[]; diagramType: string; document: DiagramDocument }) {
  ...
  const manualRouting = edge.data.routingMode === 'manual' || edge.waypoints.length > 0;
  ...
  const toggleRouting = () => patch({ data: { routingMode: manualRouting ? 'auto' : 'manual' }, ...(manualRouting ? { waypoints: [] } : {}) }, manualRouting ? 'Use automatic routing' : 'Use manual routing');
...
function EdgeStyleControls({ edge, document, patch }: { edge: DiagramEdge; document: DiagramDocument; patch: (changes: EdgePatch, action?: string) => void }) {
  const updatePalette = useEditorStore((state) => state.updatePalette);
  return <div className="style-controls"><ColorField label="Stroke" value={edge.style.stroke} colors={document.palette} onChange={(color) => patch({ style: { stroke: color } }, 'Change connector color')} onAddToPalette={() => updatePalette([...document.palette, edge.style.stroke])} /><div className="style-range-row"><label className="field-label">Stroke width<input aria-label="Connector stroke width" type="number" min="0" max="100" step="0.5" value={edge.style.strokeWidth} onChange={(event) => patch({ style: { strokeWidth: Number(event.target.value) } }, 'Change connector stroke width')} /></label><label className="field-label">Opacity<input aria-label="Connector opacity" type="range" min="0" max="1" step="0.05" value={edge.style.opacity ?? 1} onChange={(event) => patch({ style: { opacity: Number(event.target.value) } }, 'Change connector opacity')} /></label></div><label className="field-label edge-field-label">Line jump<select className="inspector-select" value={edge.style.jumpStyle ?? 'arc'} onChange={(event) => patch({ style: { jumpStyle: event.target.value as EdgeStyle['jumpStyle'] } }, 'Change line jump style')}><option value="arc">Arc</option><option value="gap">Gap</option><option value="none">None</option></select></label></div>;
}
