...
import { createEntityField, entityAutoHeight, entityVariantOptions, normalizeEntityFields, parseEntityFields, validateErd } from '../core/erd';
import { editorEvents } from '../core/events';
import { nodeCenter } from '../core/geometry';
import type { DiagramDocument, DiagramEdge, DiagramNode, EdgeMarker, EdgePatch, EdgeStyle, Endpoint, NodeStyle, Point } from '../core/types';
...
function EntityAdvancedFields({ fields, entities, updateFields }: { fields: EntityField[]; entities: DiagramNode[]; updateFields: (fields: EntityField[], action?: string) => void }) {
...
function NodePropertiesPanel({ node, document, activePageId }: { node: DiagramNode; document: DiagramDocument; activePageId: string }) {
  ...
  return <aside className="properties-panel">
    ...
    <InspectorSection id="node.layout" title="Layout"><div className="coordinate-grid"><label className="field-label">X<input type="number" value={Math.round(node.position.x)} onChange={(event) => updateNode(node.id, { position: { ...node.position, x: Number(event.target.value) } }, 'Move node')} /></label><label className="field-label">Y<input type="number" value={Math.round(node.position.y)} onChange={(event) => updateNode(node.id, { position: { ...node.position, y: Number(event.target.value) } }, 'Move node')} /></label><label className="field-label">W<input type="number" value={Math.round(node.size.width)} onChange={(event) => updateNode(node.id, { size: { ...node.size, width: Number(event.target.value) } }, 'Resize node')} /></label><label className="field-label">H<input type="number" value={Math.round(node.size.height)} onChange={(event) => updateNode(node.id, { size: { ...node.size, height: Number(event.target.value) } }, 'Resize node')} /></label></div></InspectorSection>
     ...
     {isErdEntity && <EntityAdvancedFields fields={entityFields} entities={page?.nodes.filter((candidate) => candidate.type === 'entity' && candidate.id !== node.id) ?? []} updateFields={updateFields} />}
...
function DfdMetadataPanel({ node }: { node: DiagramNode }) {
  const updateNode = useEditorStore((state) => state.updateNode);
  return <InspectorSection id="node.dfd-number" title="DFD metadata"><label className="field-label">Process number<input aria-label="Process number" value={typeof node.data.number === 'string' ? node.data.number : ''} placeholder="1.0" onChange={(event) => updateNode(node.id, { data: { number: event.target.value } }, 'Edit process number')} /></label><span className="inspector-hint">Use hierarchical numbers such as 1.0, 1.1, or 2.0 when balancing DFD levels.</span></InspectorSection>;
}
