...
function EmptyProperties({ document, page }: { document: DiagramDocument; page?: DiagramPage }) {
 ...
 function PagePaperProperties({ page }: { page: DiagramPage }) {
   const updatePageSettings = useEditorStore((state) => state.updatePageSettings);
   const presets = [
     { id: 'a4', label: 'A4', width: 794, height: 1123 },
     { id: 'letter', label: 'Letter', width: 816, height: 1056 },
     { id: 'a3', label: 'A3', width: 1123, height: 1587 },
     { id: 'slide', label: '16:9 slide', width: 1600, height: 900 },
   ];
   const current = presets.find((preset) => preset.width === page.settings.width && preset.height === page.settings.height);
   const orientation = page.settings.width >= page.settings.height ? 'landscape' : 'portrait';
   return <InspectorSection id="page.paper" title="Page / print">
     <label className="field-label">Page size<select className="inspector-select" aria-label="Page size" value={current?.id ?? 'custom'} onChange={(event) => { const preset = presets.find((candidate) => candidate.id === event.target.value); if (preset) updatePageSettings({ width: preset.width, height: preset.height }, page.id, `Use ${preset.label} page`); }}><option value="custom">Custom</option>{presets.map((preset) => <option key={preset.id} value={preset.id}>{preset.label}</option>)}</select></label>
     <label className="field-label">Orientation<select className="inspector-select" aria-label="Page orientation" value={orientation} onChange={(event) => { if (event.target.value !== orientation) updatePageSettings({ width: page.settings.height, height: page.settings.width }, page.id, 'Change page orientation'); }}><option value="portrait">Portrait</option><option value="landscape">Landscape</option></select></label>
     <span className="inspector-hint">The page outline is shown on the canvas and matches page/PDF export bounds.</span>
   </InspectorSection>;
 }
  ...
  return <aside className="properties-panel empty-properties">
    ...
    <div className="multi-selection"><div className="multi-icon"><AlignCenter size={20} /></div><strong>{selectedIds.length} objects selected</strong><span>{nodes.length > 0 ? 'Edit shared properties or arrange them together' : 'Move or arrange them together'}</span></div>
     ...
     <InspectorSection id="selection.arrange" title="Arrange"><div className="arrange-actions">{arrangeModes.map((mode) => <button key={mode} onClick={() => { void autoLayout(mode); }}>{mode.replace('-', ' ')}</button>)}</div></InspectorSection>
       <div className="quick-actions"><button title="Align left" onClick={() => alignSelection('left')}><AlignCenter size={14} /> Left</button><button title="Align centers" onClick={() => alignSelection('center')}><AlignCenter size={14} /> Center</button><button title="Align right" onClick={() => alignSelection('right')}><AlignCenter size={14} /> Right</button><button title="Align top" onClick={() => alignSelection('top')}><AlignCenter size={14} /> Top</button><button title="Align middle" onClick={() => alignSelection('middle')}><AlignCenter size={14} /> Middle</button><button title="Align bottom" onClick={() => alignSelection('bottom')}><AlignCenter size={14} /> Bottom</button><button title="Distribute horizontally" onClick={() => distributeSelection('horizontal')}><AlignCenter size={14} /> Dist. H</button><button title="Distribute vertically" onClick={() => distributeSelection('vertical')}><AlignCenter size={14} /> Dist. V</button><button title="Fit shapes to text" onClick={fitSelectionToText}>Fit text</button><button title="Match selected widths" onClick={() => matchSelectionSize('width')}>Match W</button><button title="Match selected heights" onClick={() => matchSelectionSize('height')}>Match H</button><button title="Match complete size" onClick={() => matchSelectionSize('both')}>Match size</button><button title="Reset selected rotation" onClick={resetSelectionRotation}>Reset rotation</button><button title="Flip selected shapes horizontally" onClick={() => flipSelection('horizontal')}>Flip H</button><button title="Flip selected shapes vertically" onClick={() => flipSelection('vertical')}>Flip V</button><button title="Fit selection" onClick={() => editorEvents.emit('viewport:fit', { scope: 'selection' })}>Fit selection</button><button title="Duplicate selection" onClick={() => duplicateSelection()}><Copy size={14} /> Duplicate</button><button title="Group selection" onClick={groupSelection}>Group</button><button title="Ungroup selection" onClick={ungroupSelection}>Ungroup</button><button title="Bring selection to front" onClick={() => setZOrder('front')}>Bring front</button><button title="Send selection to back" onClick={() => setZOrder('back')}>Send back</button></div>
  </aside>;
}
...
function StyleControls({ nodes, document }: { nodes: DiagramNode[]; document: DiagramDocument }) {
  ...
  const setStyle = (style: Partial<NodeStyle>, label: string) => updateNodes(nodeIds, { style }, label);
   const documentColors = [...new Set(document.palette)];
   const recentOnly = recentColors.filter((color) => !documentColors.includes(color));
  ...
  return <div className="style-controls">
     <ColorField label="Fill" value={fill} documentColors={documentColors} recentColors={recentOnly} onChange={(color) => colorChange('fill', color, 'Change fill')} onAddToPalette={() => fill && updatePalette([...document.palette, fill])} />
     <ColorField label="Stroke" value={stroke} documentColors={documentColors} recentColors={recentOnly} onChange={(color) => colorChange('stroke', color, 'Change stroke')} onAddToPalette={() => stroke && updatePalette([...document.palette, stroke])} />
     <ColorField label="Text" value={textColor} documentColors={documentColors} recentColors={recentOnly} onChange={(color) => colorChange('textColor', color, 'Change text color')} onAddToPalette={() => textColor && updatePalette([...document.palette, textColor])} />
...
function ColorField({ label, value, documentColors, recentColors, onChange, onAddToPalette }: { label: string; value?: string; documentColors: string[]; recentColors: string[]; onChange: (color: string) => void; onAddToPalette: () => void }) {
  const pickerValue = value && /^#[0-9a-f]{6}$/i.test(value) ? value : '#171b28';
  return <div className="color-field"><div className="color-field-heading"><span>{label}</span><button className="palette-add" title={`Add ${label.toLowerCase()} to document palette`} aria-label={`Add ${label.toLowerCase()} to document palette`} onClick={onAddToPalette}><Plus size={11} /></button></div><div className="color-input-row"><input aria-label={`${label} color picker`} title="Open color picker" type="color" value={pickerValue} onChange={(event) => onChange(event.target.value)} /><input aria-label={`${label} color value`} className="color-value-input" value={value ?? ''} placeholder="Mixed" onChange={(event) => { if (event.target.value.trim()) onChange(event.target.value); }} /></div>{documentColors.length > 0 && <><span className="color-palette-label">Document colors</span><div className="color-palette">{documentColors.map((color) => <button key={color} aria-label={`Set ${label.toLowerCase()} ${color}`} title={color} className={`swatch ${value === color ? 'selected' : ''}`} style={{ background: color }} onClick={() => onChange(color)} />)}</div></>}{recentColors.length > 0 && <><span className="color-palette-label">Recent</span><div className="color-palette">{recentColors.map((color) => <button key={color} aria-label={`Set ${label.toLowerCase()} ${color}`} title={color} className={`swatch ${value === color ? 'selected' : ''}`} style={{ background: color }} onClick={() => onChange(color)} />)}</div></>}</div>;
}
...
function SelectionStyleActions({ nodes, document }: { nodes: DiagramNode[]; document: DiagramDocument }) {
  ...
  const setDefaultStyle = useEditorStore((state) => state.setDefaultStyle);
  const clearDefaultStyle = useEditorStore((state) => state.clearDefaultStyle);
  const [presetId, setPresetId] = useState('');
  const source = nodes[0];
  const defaultKey = source ? `${source.library}:${source.type}` : '';
  const hasDefault = Boolean(defaultKey && document.styleDefaults?.[defaultKey]);
  const savePreset = () => {
    const name = globalThis.prompt('Style preset name', nodes[0]?.type ?? 'Custom style');
    if (name?.trim()) saveStylePreset(name);
  };
  return <div className="style-actions"><div className="style-action-row"><button title="Copy style" onClick={copyStyle}><Clipboard size={13} /> Copy</button><button title="Paste style" disabled={!styleClipboard} onClick={pasteStyle}><ClipboardPaste size={13} /> Paste</button><button title={formatPainter ? 'Cancel format painter' : 'Format painter'} className={formatPainter ? 'active' : ''} onClick={() => formatPainter ? clearFormatPainter() : activateFormatPainter()}><Brush size={13} /> {formatPainter ? 'Cancel' : 'Paint'}</button></div><div className="style-action-row"><button title="Reset formatting" onClick={resetFormatting}><RotateCcw size={13} /> Reset</button><button title="Apply this style to all matching shapes" onClick={applyStyleToSameType}>Same type</button><button title="Save style preset" onClick={savePreset}><Save size={13} /> Save</button></div><div className="style-action-row"><button title={`Use this as the default ${source?.type ?? 'shape'} style`} onClick={setDefaultStyle} disabled={!source}>Set default</button>{hasDefault && <button title={`Clear the default ${source?.type ?? 'shape'} style`} onClick={clearDefaultStyle}>Clear default</button>}</div>{document.stylePresets.length > 0 && <div className="preset-row"><select aria-label="Style preset" className="inspector-select" value={presetId} onChange={(event) => { setPresetId(event.target.value); if (event.target.value) applyStylePreset(event.target.value); }}><option value="">Apply preset…</option>{document.stylePresets.map((preset) => <option key={preset.id} value={preset.id}>{preset.name}</option>)}</select>{presetId && <button className="field-delete" title="Delete style preset" aria-label="Delete style preset" onClick={() => { deleteStylePreset(presetId); setPresetId(''); }}><Trash2 size={11} /></button>}</div>}</div>;
}
...
function aspectSizeFromWidth(width: number, ratio: number): { width: number; height: number } {
  const constrainedWidth = Math.max(48, Math.round(width));
  const constrainedHeight = Math.max(32, Math.round(constrainedWidth / ratio));
  return {
    width: Math.max(48, Math.round(constrainedHeight * ratio)),
    height: constrainedHeight,
  };
}

function aspectSizeFromHeight(height: number, ratio: number): { width: number; height: number } {
  const constrainedHeight = Math.max(32, Math.round(height));
  const constrainedWidth = Math.max(48, Math.round(constrainedHeight * ratio));
  return {
    width: constrainedWidth,
    height: Math.max(32, Math.round(constrainedWidth / ratio)),
  };
}
...
function NodePropertiesPanel({ node, document, activePageId }: { node: DiagramNode; document: DiagramDocument; activePageId: string }) {
  ...
  const intrinsicAspectRatio = shapeDefinition?.aspectRatio;
  ...
  const page = getActivePage(document, activePageId);
  const [aspectRatioLocked, setAspectRatioLocked] = useState(intrinsicAspectRatio !== undefined);
  useEffect(() => setAspectRatioLocked(intrinsicAspectRatio !== undefined), [intrinsicAspectRatio, node.id]);
  const resizeRatio = aspectRatioLocked ? intrinsicAspectRatio ?? node.size.width / Math.max(1, node.size.height) : undefined;
  ...
  return <aside className="properties-panel">
     ...
     <InspectorSection id="node.content" title="Content"><label className="field-label">Label<input value={label} onChange={(event) => updateLabel(event.target.value)} /></label><button className="secondary-button" title="Fit this shape to its text" onClick={fitSelectionToText}>Fit shape to text</button></InspectorSection>
    ...
    {isErdEntity && <InspectorSection id="node.attributes" title="Attributes" className="entity-properties" action={<button className="property-add" title="Add attribute" aria-label="Add attribute" onClick={() => updateFields([...entityFields, createEntityField()], 'Add attribute')}><Plus size={13} /></button>}><div className="entity-field-list">{entityFields.map((field, index) => <div className="entity-field-row" key={field.id}><div className="entity-field-inputs"><input aria-label={`Attribute ${index + 1} name`} value={field.name} onChange={(event) => updateField(index, { name: event.target.value })} /><input aria-label={`Attribute ${index + 1} type`} value={field.type} onChange={(event) => updateField(index, { type: event.target.value })} /></div><div className="entity-field-actions"><button className={field.primaryKey ? 'field-flag active' : 'field-flag'} title="Primary key" onClick={() => updateField(index, { primaryKey: !field.primaryKey })}><KeyRound size={11} /></button><button className={field.foreignKey ? 'field-flag active' : 'field-flag'} title="Foreign key" onClick={() => updateField(index, { foreignKey: !field.foreignKey })}><Link2 size={11} /></button><button className={field.nullable ? 'field-flag' : 'field-flag active'} title="Nullable" onClick={() => updateField(index, { nullable: !field.nullable })}>N</button><button className="field-delete" title="Delete attribute" onClick={() => updateFields(entityFields.filter((_, fieldIndex) => fieldIndex !== index), 'Delete attribute')}><Trash2 size={11} /></button></div></div>)}</div><div className="entity-style-toggle"><span>Striped rows</span><button title="Toggle row striping" aria-label="Toggle row striping" className={node.data.striped !== false ? 'toggle-button active' : 'toggle-button'} onClick={() => updateNode(node.id, { data: { striped: node.data.striped === false } }, 'Toggle row striping')}><span /></button></div>{diagnostics.map((diagnostic) => <div className={`erd-diagnostic ${diagnostic.severity}`} key={diagnostic.message}>{diagnostic.message}</div>)}</InspectorSection>}
     ...
     <InspectorSection id="node.appearance" title="Appearance"><StyleControls nodes={[node]} document={document} /><SelectionStyleActions nodes={[node]} document={document} /></InspectorSection>
      <div className="property-bottom-actions"><button title={node.locked ? 'Unlock' : 'Lock'} onClick={() => updateNode(node.id, { locked: !node.locked }, node.locked ? 'Unlock node' : 'Lock node')}>{node.locked ? <Unlock size={15} /> : <Lock size={15} />}</button><button title="Rotate clockwise" aria-label="Rotate clockwise" disabled={node.locked} onClick={() => useEditorStore.getState().rotateSelection(90)}><RotateCw size={15} /></button><button title="Flip horizontally" aria-label="Flip horizontally" disabled={node.locked} onClick={() => useEditorStore.getState().flipSelection('horizontal')}>↔</button><button title="Flip vertically" aria-label="Flip vertically" disabled={node.locked} onClick={() => useEditorStore.getState().flipSelection('vertical')}>↕</button>{isErdEntity && <button title="Duplicate table" aria-label="Duplicate table" onClick={() => duplicateSelection()}><Copy size={15} /></button>}<button title="Delete" className="danger" onClick={deleteSelection}><Trash2 size={15} /></button></div>
