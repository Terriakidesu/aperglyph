...
import { createEntityField, normalizeEntityFields, validateErd } from '../core/erd';
import { nodeCenter } from '../core/geometry';
import type { DiagramEdge, DiagramNode, EdgeMarker, EdgePatch, EdgeStyle, Point } from '../core/types';
...
export function PropertiesPanel() {
  ...
  if (selectedEdge) return <EdgePropertiesPanel edge={selectedEdge} nodes={page?.nodes ?? []} diagramType={document.diagramType} />;
...
function NodePropertiesPanel({ node, document }: { node: DiagramNode; document: ReturnType<typeof useEditorStore.getState>['document'] }) {
  ...
  const updateFields = (fields: EntityField[], action = 'Update attributes') => updateNode(node.id, { data: { fields }, size: { ...node.size, height: Math.max(node.size.height, 58 + fields.length * 27) } }, action);
...
function EdgePropertiesPanel({ edge, nodes, diagramType }: { edge: DiagramEdge; nodes: DiagramNode[]; diagramType: string }) {
  ...
  const updateSource = (changes: Partial<DiagramEdge['source']>, action = 'Edit source connection') => patch({ source: { ...edge.source, ...changes } }, action);
  const updateTarget = (changes: Partial<DiagramEdge['target']>, action = 'Edit target connection') => patch({ target: { ...edge.target, ...changes } }, action);
