...
function EmptyProperties({ multiSelection }: { multiSelection?: number }) {
  ...
  return <aside className="properties-panel empty-properties">
    <InspectorHeading />
    {multiSelection ? <div className="multi-selection"><div className="multi-icon"><AlignCenter size={20} /></div><strong>{multiSelection} objects selected</strong><span>Move or align them together</span><div className="quick-actions"><button title="Align left" onClick={() => alignSelection('left')}><AlignCenter size={14} /> Align left</button><button title="Align centers" onClick={() => alignSelection('center')}><AlignCenter size={14} /> Align center</button><button title="Distribute horizontally" onClick={() => distributeSelection('horizontal')}><AlignCenter size={14} /> Distribute</button><button title="Duplicate selection" onClick={duplicateSelection}><Copy size={14} /> Duplicate</button><button onClick={groupSelection}>Group</button><button onClick={ungroupSelection}>Ungroup</button><button onClick={() => setZOrder('front')}>Bring front</button><button onClick={() => setZOrder('back')}>Send back</button></div></div> : <div className="no-selection"><div className="no-selection-art"><div /><div /><div /></div><strong>Nothing selected</strong><span>Select an object to<br />inspect its properties.</span></div>}
  </aside>;
}
