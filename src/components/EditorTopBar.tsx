...
type ExportArea = 'page' | 'content' | 'selection';
...
export function EditorTopBar({ onExit, onDiagnostics, diagnosticCount = 0, view, gridVisible, onViewChange, onToggleGrid, onFitPage, onFitSelection, onZoom100, onToggleFocus, onToggleFullscreen, onTogglePresentation, fullscreen, focusMode, presentationMode, onOpenDocument, onOpenPreferences }: EditorTopBarProps) {
  ...
  const [exportArea, setExportArea] = useState<ExportArea>('content');
...
function ExportMenu({ document, activePageId, selectionIds, area, scale, padding, transparent, outlineOnly, setArea, setScale, setPadding, setTransparent, setOutlineOnly, close }: { document: Parameters<typeof downloadProject>[0]; activePageId: string; selectionIds?: string[]; area: ExportArea; scale: ExportScale; padding: number; transparent: boolean; outlineOnly: boolean; setArea: (area: ExportArea) => void; setScale: (scale: ExportScale) => void; setPadding: (padding: number) => void; setTransparent: (transparent: boolean) => void; setOutlineOnly: (outlineOnly: boolean) => void; close: () => void }) {
  const useSelection = area === 'selection' && Boolean(selectionIds);
  const contentBounds = area !== 'page';
  const selected = useSelection ? selectionIds : undefined;
  return <div className="topbar-menu export-menu" onPointerDown={(event) => event.stopPropagation()}>
    <strong>Export diagram</strong>
    <div className="export-area" role="group" aria-label="Export area"><span>Area</span><button className={area === 'page' ? 'active' : ''} onClick={() => setArea('page')}>Page</button><button className={area === 'content' ? 'active' : ''} onClick={() => setArea('content')}>Content</button><button disabled={!selectionIds} className={area === 'selection' ? 'active' : ''} onClick={() => selectionIds && setArea('selection')}>Selection</button></div>
    ...
    <button onClick={() => { printDocument(document, { pageId: activePageId, contentBounds, padding, selectionIds: selected, outlineOnly: true }); close(); }}>Print current page · outline-only</button>
    <button onClick={() => { downloadSvg(document, activePageId, { contentBounds, padding, transparent, outlineOnly, selectionIds: selected }); close(); }}>{useSelection ? 'SVG · selection' : 'SVG · current page'}</button>
    <button onClick={() => { downloadSvg(document, activePageId, { allPages: true, contentBounds, padding, transparent, outlineOnly }); close(); }}>SVG · all pages</button>
    ...
    <button onClick={() => { void downloadPng(document, { pageId: activePageId, contentBounds, padding, selectionIds: selected, scale, transparent, outlineOnly }); close(); }}>{useSelection ? 'PNG · selection' : 'PNG · current page'}</button>
    <button onClick={() => { void downloadPng(document, { allPages: true, contentBounds, padding, scale, transparent, outlineOnly }); close(); }}>PNG · all pages</button>
    <button onClick={() => { void downloadPdf(document, { pageId: activePageId, contentBounds, padding, selectionIds: selected, scale, transparent, outlineOnly }); close(); }}>{useSelection ? 'PDF · selection' : 'PDF · current page'}</button>
    <button onClick={() => { void downloadPdf(document, { contentBounds, padding, scale, transparent, outlineOnly }); close(); }}>PDF · all pages</button>
  </div>;
}
