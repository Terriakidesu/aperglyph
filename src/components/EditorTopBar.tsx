...
function ExportMenu({ document, activePageId, selectionIds, scale, transparent, setScale, setTransparent, close }: { document: Parameters<typeof downloadProject>[0]; activePageId: string; selectionIds?: string[]; scale: ExportScale; transparent: boolean; setScale: (scale: ExportScale) => void; setTransparent: (transparent: boolean) => void; close: () => void }) {
  return <div className="topbar-menu export-menu" onPointerDown={(event) => event.stopPropagation()}>
    ...
    <button onClick={() => { downloadSvg(document, activePageId, { contentBounds: true, transparent }); close(); }}>SVG · current page</button>
    <button onClick={() => { void downloadPng(document, { pageId: activePageId, contentBounds: true, selectionIds, scale, transparent }); close(); }}>{selectionIds ? 'PNG · selection' : 'PNG · current page'}</button>
    <button onClick={() => { void downloadPng(document, { allPages: true, contentBounds: true, scale, transparent }); close(); }}>PNG · all pages</button>
    <button onClick={() => { void downloadPdf(document, { pageId: activePageId, contentBounds: true, selectionIds, scale, transparent }); close(); }}>{selectionIds ? 'PDF · selection' : 'PDF · current page'}</button>
    <button onClick={() => { void downloadPdf(document, { contentBounds: true, scale, transparent }); close(); }}>PDF · all pages</button>
  </div>;
}
