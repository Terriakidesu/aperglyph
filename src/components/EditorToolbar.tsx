import { Hand, MousePointer2, Shapes, Spline, Type } from 'lucide-react';
...
export function EditorToolbar() {
  ...
  return <aside className="tool-rail" aria-label="Editor tools">
    <div className="tool-group main-tools" role="toolbar" aria-label="Canvas tools">
      {tools.map(({ id, label, icon: Icon, shortcut }) => <button key={id} className={`rail-button ${activeTool === id ? 'active' : ''}`} onClick={() => setTool(id)} title={`${label} (${shortcut})`} aria-label={`${label} tool`} aria-pressed={activeTool === id} data-tool={id}><Icon size={18} /><span className="rail-label">{label}</span><kbd className="rail-shortcut">{shortcut}</kbd>{activeTool === id && <i aria-hidden="true" />}</button>)}
    </div>
