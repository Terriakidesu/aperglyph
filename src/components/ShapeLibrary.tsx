...
export function ShapeLibrary() {
  ...
  return <aside className="shape-library">
    <div className="panel-title-row shape-library-heading"><div><span className="panel-kicker">Library</span><h2>Shapes</h2></div><span className="library-count">{plugins.reduce((count, plugin) => count + plugin.shapes.length, 0)}</span></div>
    ...
    <div className="library-filters" role="tablist" aria-label="Shape library filters">
      {(['all', 'favorites', 'recent'] as LibraryFilter[]).map((value) => <button key={value} role="tab" aria-selected={filter === value} className={filter === value ? 'active' : ''} onClick={() => setFilter(value)}>{value === 'all' ? 'All' : value === 'favorites' ? 'Favorites' : 'Recent'}</button>)}
    </div>
    <div className="library-scroll">
      {plugins.map((plugin) => {
        ...
        return <ShapeGroup key={plugin.id} title={plugin.id === 'general' ? 'Basic shapes' : `${plugin.name} shapes`} shapes={shapes} open={open} onToggle={() => {
          ...
          setOpenGroups(next);
          writeGroups(next);
        }} onAdd={(shape) => addShape(shape, plugin.id)} libraryId={plugin.id} isFavorite={isFavorite} onToggleFavorite={toggleFavorite} />;
      })}
