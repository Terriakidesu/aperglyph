...
export function StatusBar() {
  ...
  return <div className="status-bar"><div className="status-left"><span className="status-mode"><span className="status-dot" /> Ready</span><span className="status-divider" /><button className="status-toggle" title="Toggle snapping" onClick={() => updatePageSettings({ snapToGrid: !(page?.settings.snapToGrid ?? true) }, page?.id, 'Toggle snap')}><Grid3X3 size={13} /> Snap {page?.settings.snapToGrid ? 'on' : 'off'}</button><button className="status-toggle" title="Change grid settings" onClick={() => updatePageSettings({ gridVisible: !(page?.settings.gridVisible ?? true) }, page?.id, 'Toggle grid')}><Ruler size={13} /> {page?.settings.gridSize ?? 16} px grid</button></div><div className="status-right"><span className="shortcut-hint"><Command size={12} /> K for shortcuts</span><span className="status-divider" /><div className="zoom-control"><button onClick={() => updateViewport({ zoom: Math.max(.25, viewport.zoom - .1) })}><Minus size={13} /></button><span>{zoom}%</span><button onClick={() => updateViewport({ zoom: Math.min(3, viewport.zoom + .1) })}><Plus size={13} /></button></div><button className="help-button" title="Keyboard shortcuts" onClick={() => editorEvents.emit('ui:shortcuts', undefined)}><HelpCircle size={15} /></button></div></div>;
}
