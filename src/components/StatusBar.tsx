  ...
  const snappingEnabled = Object.values(snapSettings).some(Boolean);
   return <div className="status-bar">
     <div className="status-left">
       <span className={`status-save ${isDirty ? 'dirty' : ''}`} title={isDirty ? 'Changes are saved automatically' : 'Saved locally'}><i className="status-dot" /> {isDirty ? 'Saving locally…' : 'Saved locally'}</span>
       <span className="status-local"><CloudOff size={12} /> Offline</span>
     </div>
     <div className="status-center">
       <span className="status-mode" title={`Current tool: ${toolLabels[activeTool]}`}>{toolLabels[activeTool]} tool</span>
       <span className="status-divider" />
       <span className="status-selection">{selectedCount > 0 ? `${selectedCount} selected` : `${page?.nodes.length ?? 0} objects`}</span>
       <span className="status-stat">{page?.edges.length ?? 0} connectors</span>
